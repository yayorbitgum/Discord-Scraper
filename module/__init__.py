# Entering my username as the author of the library.
__author__ = 'Dracovian'

# You can contact me via Github, we can set up other forms of communication if needed.
__contact__ = 'https://github.com/Dracovian'

# There's no copyright information for this.
__copyright__ = None

# November 7, 2020 (ISO 8601 date formatting)
__date__ = '2020-11-09'

# Crediting those who have contributed to the Discord Scraper repository on Github which showcased this library for the first time.
__credits__ = ['https://github.com/mukatee', 'https://github.com/MRosenst']

# The license for this library is the same as the Discord Scraper repository.
__license__ = 'WTFPL'

# The current version of this library, it's 1.0-Beta now instead of 1.0-Alpha from the previous iterations on the repository.
__version__ = '1.0.2'

# So far I'm the sole maintainer of this library, this is apt to change if anyone decides to create a pull request that ends up merged for this library.
__maintainer__ = 'Dracovian'

# I do not wish to set up an e-mail communication, I don't need any more spam mail.
__email__ = None

# I'm not 100% certain what this means, but I suppose this library is production ready since it's in use on more than one computer.
__status__ = 'Production'

# Time to create a simple alias for the library when importing it.
from .DiscordScraper import DiscordScraper
